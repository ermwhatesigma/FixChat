...
name_chat_random = ["Homie", "Best Friend", "Girlfriend", "Boyfriend", "Dumb Homie", "Torta", "Huzz", "Buzz", "Chuzz", "Fine Shit", "The Fridge", "Walking Wallet", "The Fine Bro", "Quarter Pounder", "Fat Homie", "**Friend**", "The Home Girl", "Girl Bestfriend", "Guy Bestfriend", "Dominant Mommy", "Femboy twinky", "Tomboy", "Dommy Mommy", "Mommy", "Good Boy", "Good Girl"]
all_genders = ["Female", "Male"]

def load_api_keys():
    keys = []
    try:
        with open("OpenAI_API.csv", "r", encoding="utf-8") as f:
            reader = csv.DictReader(f)
            for row in reader:
                keys.append(row["API_Key"])
    except FileNotFoundError:
        print("No OpenAI_API.csv found")
    return keys
...
for i, _ in enumerate(api_keys, start=1):
    print(i)

selected_index = int(input("> ")) - 1
ACTIVE_API_KEY = api_keys[selected_index]
...
def chat_worker(history):
    ...
    try:
        ...
        pending_ai_message = completion.choices[0].message.content
    except Exception as e:
        pending_ai_message = f"Error: {e}"
    is_thinking = False

def diagnostic_worker(history):
    global pending_ai_message, is_thinking
...
while running:
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
            ...
            if switch_btn.collidepoint(mx, my) and not is_thinking:
                print("Select API key:")
                for i, _ in enumerate(api_keys, start=1):
                    print(i)
                try:
                    idx = int(input("> ")) - 1
                    ACTIVE_API_KEY = api_keys[idx]
                    selected_index = idx
                    client = OpenAI(
                        base_url="https://openrouter.ai/api/v1",
                        api_key=ACTIVE_API_KEY,
    ...
    header_text = font.render(f"{chat_name} (API {selected_index+1})", True, (255,255,255))
    ...
    switch_btn = pygame.Rect(WIDTH-110, 10, 100, 30)
    pygame.draw.rect(screen, (70,70,70), switch_btn, border_radius=8)
    switch_text = font.render("Change API", True, (255,255,255))
    screen.blit(switch_text,
           (switch_btn.centerx - switch_text.get_width()//2,
            switch_btn.centery - switch_text.get_height()//2))
